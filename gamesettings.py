    ...
    def render(self, active=False):
        ...
        if self.controller:
            ...
            surface.blit(
                ctrl,
                (
                    surface.get_width() - ctrl.get_width(),
                    surface.get_height() / 2 - ctrl.get_height() / 2,
                ),
            )
        return surface
...
def get_settings_menu():
    settings_menu = [
        ...
        SettingItem(
            "Music level", "music_volume", settings["music_volume"], Slider(0, 10, 1)
        ),
        ...
        for event in events:
            if self.escdialog.visible:
                ...
                if event.key == pygame.K_DOWN:
                    ...
                    and self.menu[self.selected].key == "save"
                ):
